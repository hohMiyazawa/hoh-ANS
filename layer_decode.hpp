...
void decode_layer_simple(
	...
	if(compaction_mode == 0){
		...
		if(bit_depth == 8){
			...
			printf("[SIMPLE] unimplemented bit depth!\n");
		}
	}
	else if(compaction_mode == 2){
		printf("[SIMPLE]     bitmasked channel\n");
		//bitmask
	...
	printf("[SIMPLE] prediction mode: %d\n",prediction_mode);
	...
	if(prediction_mode){
		...
		if(x_tiles == 1 && y_tiles == 1){
			...
			printf("[SIMPLE]     unimplemented prediction mode!\n");
